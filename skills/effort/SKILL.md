...
Then check the tracker, per `## Tracker` above, before the interview rather than after it: if `capstan-tracker` names a GitHub surface and the `capstan-board:tracker` skill cannot be loaded, stop here the same way that section says — install `capstan-board@bytesnation`, reload plugins, and re-run this phase. Every project that reaches this stop already has a board configured, so `/capstan-board:setup` is not the next step. Waiting until a later phase to find this out wastes the gates run in between.
