...
A `Bash` deny rule also reaches past the exact command shape it names. Claude Code splits a compound command on `&&`, `||`, `;`, `|`, `|&`, `&`, and newlines, and checks each piece on its own, so `Bash(git stash *)` still blocks `cd /tmp && git stash`. It also strips a fixed set of wrappers before matching, including `timeout`, `nohup`, and bare `xargs`, so `Bash(rm -rf*)` still blocks `timeout 5 rm -rf tmp/`. A leading `VAR=value` assignment is stripped the same way, so it still blocks `FOO=1 rm -rf tmp/`.
...
- The matcher has no one-word wildcard, so the `-C` rules above also match past the command they name: any `git -C <dir>` command whose message contains "stash", or "push" followed by "-f" or "--force", is denied too, including a commit that only mentions those words. Exact parsing of the command needs a [PreToolUse hook](https://code.claude.com/docs/en/permissions#extend-permissions-with-hooks) instead, which inspects the full command text before it runs.
- `Bash(gh auth login *)`, `logout`, `refresh`, and `switch` stop the four subcommands that change which account is signed in. `gh auth token` prints the token to stdout, so `Bash(gh auth token *)` blocks a credential read rather than a session change. `Bash(gh auth setup-git *)` stops `gh` from rewriting git's credential helper. `Bash(gh auth status -t*)` and its three siblings stop `gh auth status --show-token` and its `-t` short form, which print the same token `gh auth token` does; a plain `gh auth status` stays open, since it only reads state. So do `gh auth status -at`, whose bundled short flags reach the same token through `-a` and `-t` together, and `gh auth git-credential get`, the credential helper `gh auth setup-git` installs, which returns the token under git's credential protocol.
...
The `.env` and `.env.*` rules anchor at the filesystem root with `//**/`, so they block a `.env` file anywhere on the machine, not only inside the current project. `.env.*` also blocks `.env.example` and `.env.sample`, the templates most projects check into version control. The docs' own carve-out, a `!` rule such as `Read(!sample.env)`, cannot reach a rule anchored with `//`, `~/`, or `/`: Claude Code reads a `!` pattern relative to the current directory regardless of what prefix follows it, and an allow rule can't carve an exception out of a deny rule in any case. Exempting a template file needs one of two changes: drop the `.env.*` line and keep only `.env`; or, in this same file, replace `Read(//**/.env.*)` and `Edit(//**/.env.*)` with the cwd-relative `Read(.env.*)` and `Edit(.env.*)`, followed by `Read(!.env.example)` and `Edit(!.env.example)`. `.env.*` is then covered only under the current directory.
