...
**Baseline arm.** Plain Claude Code, `capstan@bytesnation` and `capstan-board@bytesnation` disabled, given the task statement above and nothing else. Let it work until it says it is done, or gets stuck.
...
Before the baseline session gets the task, open `/plugin` in that session and confirm neither `capstan@bytesnation` nor `capstan-board@bytesnation` shows as enabled. A plugin that is not installed passes this check. If either shows as enabled, stop: the settings file did not take effect, most likely because managed settings force it on, and the baseline arm cannot start until the operator resolves that.
...
3. Install dependencies, network allowed:
   ...
   - cobra: `go mod download` in the clone, then `GOBIN=$HOME/go/bin go install github.com/kyoh86/richgo@latest` and add `$HOME/go/bin` to `PATH`. Source: `.github/workflows/test.yml` on `spf13/cobra`, which sets `GOBIN` the same way before this install, at `test.yml:89`, so `richgo` lands where `make richtest` expects it rather than in `$GOPATH/bin`. On macOS with Go older than 1.24, CI also sets `GO_TEST_FLAGS=-ldflags=-linkmode=external`, at `test.yml:74-78`. Set it too.
...
2. Make two fresh clones of the pinned commit, one for each arm, so neither arm's work touches the other's:
   ...
   ```
3. In the baseline clone, write `.claude/settings.local.json` as shown above, confirm neither plugin shows as enabled at `/plugin`, then start the baseline arm with `claude --model <full model id>` and give it the task statement. Record `claude --version` and the start and end clock times: the timestamp of the first message and the timestamp of the model's final answer. Step 5 below locates the session file spanning this window.
...
5. Locate each arm's session file. By default it is at:
   ...
   ```
   The Capstan arm spans at least four runs, one per phase, ended at the three gates plus delivery. Only a forked reopen, `/branch` or `--fork-session`, writes its own session file. A plain resume, such as closing and reopening the terminal and running `--continue` with no fork, writes into the same file the session already had. The four runs may therefore land in one file or several, depending on whether the operator forked between gates. List every `.jsonl` file under that project directory with a modification time inside the window recorded at step 3 or 4, and treat each as a session for the next step.
6. List the session files from step 5 oldest-created first:
   ```
   ls -trU "${CLAUDE_CONFIG_DIR:-$HOME/.claude}/projects/$escaped"/*.jsonl
   ```
   which sorts by creation time on macOS. Drop from that listing any file whose modification time falls outside the window recorded at step 3 or 4. Run `bench/bin/session-usage` once per arm, passing what remains as separate arguments in one call, in the order printed: `bench/bin/session-usage <session1.jsonl> <session2.jsonl> ...`. Order matters: `session-usage` credits a turn shared by two files to whichever file is given first, so passing them out of creation order credits a fork's copied turns to the wrong file and misstates both files' spans. It dedupes by `message.id` across every file given. That matters when the operator's session was closed and reopened with `/branch` or `--continue --fork-session`, since both copy a session's opening turns into the new file. Passing every file in one call avoids counting those copied turns twice. Running each file separately and adding totals by hand would not. It also prints a first and last timestamp for each session file, counted only from the turns it kept for that file. "Wall-clock time" below sums these.
...
- **Tokens and cost, by model.** From `session-usage`, per the previous section. `session-usage` reports a model absent from `pricing.tsv` as unpriced rather than $0, and a model with one blank rate as partial rather than a silently short total. A row or the total can also carry `doubt: <reason>`. Partial and doubt never mean the same thing: partial marks a component this script could not price at all, while doubt marks a row that priced cleanly but carries a reason to look twice at what was billed. Three reasons appear: `fast mode` (`usage.speed` was not `standard`), `US-only inference` (`usage.inference_geo` was `us`, billed at a surcharge this script does not apply), and `iterations disagree` (the turn's `usage.iterations` summed to something other than its own top-level usage). Put everything `session-usage` printed before `doubt:` in `RESULTS-TEMPLATE.md`'s cost cell, exactly as printed. Carry the `doubt: <reason>` clause itself into Notes, exactly as printed, alongside which model and row it came from. A `fast mode` doubt means this protocol's fast-mode-off condition failed for that arm: re-run the arm with fast mode off, rather than keeping the figures this run produced.
- **Wall-clock time.** Sum every session file's own span, `last_timestamp` minus `first_timestamp`, as step 6's single `session-usage` call printed it for that file. A copied opening turn counts toward only the file that first carries it, so a forked file's span never repeats time that already belongs to the session it was forked from. This figure includes every gap that falls inside a file's own span, gate waits included: a session left open at a gate counts the operator's wait the same as active work. A gap between two separate files falls outside every file's span and outside this sum, because none of that gap's time lies inside either file's span. This total can therefore fall short of the calendar time between the clock times recorded at step 3 or 4.
