...
Nothing here has been run yet. Every number in `RESULTS-TEMPLATE.md` stays blank until a real run produces it.
...
2. **spf13/cobra** (Go), issue [#739](https://github.com/spf13/cobra/issues/739), "Option to pass unknown flags to command". Apache-2.0. Roughly 10-12k lines. Its last code push was about 2.5 months before this effort started, at the edge of the "active" bar the scout was asked to check. Pre-flight step 1 below confirms it is still current before it is used.
...
4. **sharkdp/bat** (Rust), issue [#1611](https://github.com/sharkdp/bat/issues/1611), "Support increments with `--highlight-line`", the spare. Apache-2.0. Roughly 12-15k lines. Use this in place of any repository above that fails its pre-flight check below.
...
- **click:** `uv run --locked --no-default-groups --group dev tox run`. Source: `.github/workflows/tests.yaml` on `pallets/click`, fetched 2026-09-22.
- **cobra:** `make richtest`. Source: `.github/workflows/test.yml` on `spf13/cobra`, fetched 2026-09-22.
...
- **bat, spare:** `cargo test --locked --release`. Source: the `test_with_new_syntaxes_and_themes` job in `.github/workflows/CICD.yml` on `sharkdp/bat`, fetched 2026-09-22. That job also checks out submodules first; see the dependency-install step below.

Use the exact command above for a repository's declared-checks measurement in every arm. It is the local test command in the sense CONTEXT.md gives that phrase, taken from CI rather than picked ad hoc.
...
- the main session's model, pinned the same way in both arms with `claude --model <model>`. Record the model used in `RESULTS-TEMPLATE.md`'s Notes. The Capstan arm's crew still runs its own frontmatter models regardless of this flag: Builder and Scout and Courier on Sonnet, Reviewer on Opus, per `agents/*.md`. Record those in Notes too, so a cost difference between arms is never read as a same-model comparison when it is not.
...
Use `false`, not a missing entry. A missing entry falls back to the plugin's own default, which is enabled.

**Arm order:** run the baseline arm first, then the Capstan arm, for every repository. This is a fixed order, not alternated, so the same variable is held constant across all three repositories. Note in `RESULTS-TEMPLATE.md` that this is the order used. Running baseline first also means the operator answering the Capstan arm's interview and gates has not yet seen that repository's own baseline result, so those answers are not shaped by watching the other arm run.

## Pre-flight, once per repository before either arm runs

1. Confirm the repository is still current. Check its GitHub page for a push within the last month. If it is not current, use the spare (bat) instead, and note the swap and the reason in `RESULTS-TEMPLATE.md`.
...
3. Install dependencies, network allowed:
   ...
   - cobra: `go mod download` in the clone.
   - commander.js: `npm ci` in the clone.
   - bat, spare: `git submodule update --init --recursive`, then `cargo fetch`, in the clone.
4. Disconnect the network. Turn off Wi-Fi, or unplug the network cable. Then run the repository's test command from the table above, timed:
   ```
   time <test command>
   ```
...
2. Make two fresh clones of the pinned commit, one for each arm, so neither arm's work touches the other's:
   ...
   ```
3. In the baseline clone, write `.claude/settings.local.json` as shown above, then start the baseline arm with `claude --model <model>` and give it the task statement. Record wall-clock time from the first message to the model's final answer.
4. In the Capstan clone, start the Capstan arm with `claude --model <model>`, then run `/capstan:effort <task statement>`. Record wall-clock time from the first message to the closing brief.
...
6. For each session file located in step 5, run `bench/bin/session-usage <session.jsonl-or-session-dir>`. When an arm used more than one session file, run `session-usage` on each and add their per-model totals by hand.
7. For each arm, run the repository's test command against the result and record pass or fail.
8. For the Capstan arm, before answering gate 3, copy `<clone>/.capstan/effort/review/` to `bench/transcripts/<repo>-capstan/review/`. Phase 4 deletes that directory before the closing brief, so it must be copied first. Read every slice's review, every round, from that copy, and count how many findings caught a real defect, by the rule below.
...
- **Gate count.** How many gates the Capstan arm's run stopped at before delivery. Write "n/a" for the baseline arm. It has no gates by definition, which is different from a gate count of zero.
...
- for the Capstan arm, the copied `review/` directory from step 8 above, and the text of every gate brief and the operator's answer to it
...
Nothing goes into `RESULTS-TEMPLATE.md` that this protocol did not measure. A blank cell means "not run yet"; it never means zero. "n/a" means the cell does not apply to that arm. Neither is ever filled from a guess, a similar-looking prior run, or what seems plausible.
