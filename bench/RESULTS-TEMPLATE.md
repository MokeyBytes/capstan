...
Filled in from real runs of `bench/PROTOCOL.md`, one row per arm per repository. Every cell starts blank. A blank cell means the run has not measured it yet. It is never filled with a guess. "n/a" means the cell does not apply to that arm, such as the baseline arm's gate count and review-findings cells, which have no gates and no review step by definition. A blank and "n/a" mean different things: never write one for the other.
