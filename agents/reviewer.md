...
- **The decision log**, `decisions.md` in the document home, which is `<working copy>/.capstan/` unless configured otherwise, if the repository keeps one. It is where the repository declines or narrows a standard that arrived with the plugin rather than with the code. Read it whole before you grade Standards, not after, then `grep -ri` `decisions/archive/` in the same document home for the name of each standard you are about to grade against — a decline that rotated out of `decisions.md` still binds, once `two-axis-review` confirms it has not itself been superseded.
