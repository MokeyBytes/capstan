...
| Agent override | `<working copy>/.claude/agents/capstan-<role>.md`, frontmatter `name: capstan-<role>`, marked by its first body line as a Capstan override and naming the plugin version it was copied from. Spawned as `capstan-<role>` in place of the plugin's `capstan:<role>`, only from a session started in the working copy. Written by `agent-models`, never by hand-editing the plugin. Plugin installs only. |
| Codex override | `.codex/agents/capstan-<role>.toml` in a working copy, written by `agent-models`, carrying the same marker as an Agent override as its first-line comment. Codex's counterpart of an Agent override. |
| Drift flag | `agent-models`' flag on an Agent or Codex override whose recorded plugin version differs from the installed plugin's. Not Drift, which is measured from the Verified commit. |
| Front door | A skill the operator invokes directly, rather than one the model reaches for. Four: `effort` starts a run, `quick` runs single-slice work through one gate, `setup` configures where its artifacts live, `agent-models` retunes the crew's models and effort. The `capstan-board` plugin adds a fifth, its own `setup`, for the Tracker surface. None is a Discipline. |
...
| Run | One invocation that ends: an effort phase, bounded by the gate it ends at, a Setup invocation, one of a Quick's two invocations, or either half of an `agent-models` invocation, split at its gate, the second begun by the operator's reply. Four phases means at least four runs, and a run is never the whole effort. What "this run's first row" is counted against.  |
