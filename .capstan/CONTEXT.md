...
| Agent override | A file in the working copy's `.claude/agents/`, named for a crew role and marked by its header as a Capstan override, naming the plugin version its body was copied from. Spawned in place of the plugin's namespaced agent. Written by `agent-models`, never by hand-editing the plugin. |
