---
capstan_type: tracker
---

# Tracker

| Effort | Slice | Status | Commit |
|---|---|---|---|
| architect-disciplines | load-point | merged | 0bd7aa9 |
| code-standards | builder-standards | merged | 15dfa0d |
| code-standards | comment-rule | merged | 4f00453 |
| code-standards | declared-checks | merged | 243d4db |
| external-review | bench | merged | 4725c2a |
| external-review | board | merged | a88173c |
| external-review | deny | merged | 9c61880 |
| external-review | log-helper | merged | 3e25155 |
| external-review | log-readers | merged | 92c09e9 |
| external-review | quick | merged | 960e9a2 |
| external-review | readme | merged | 4a3e8c5 |
| external-review | root-test | merged | 7f0b587 |
| github-surface | docs | merged | 04d33d6 |
| github-surface | effort-read | merged | b393bb7 |
| github-surface | phase-files | merged | a681858 |
| github-surface | setup-surface | merged | ace4bff |
| github-surface | tracker-mapping | merged | 6c36b2e |
| home-setup | effort-branch | merged | cdc4a80 |
| home-setup | ghost-report | merged | 2d12a69 |
| home-setup | guard | merged | 16c799e |
| home-setup | readme | merged | 7a81dfe |
| home-setup | release | merged | cbbb098 |
| home-setup | setup-skill | merged | 3cfc1a3 |
| phase4-steps | deliver-steps | merged | f6cb78b |
| pointer-gloss | enum-gloss | merged | 0820fa0 |
| qualified-paths | readme | merged | d8a2a8d |
| qualified-paths | scratch-paths | merged | 50c0892 |
| reverse-migration | format-contract | merged | 9166763 |
| reverse-migration | reverse-docs | merged | 1e8b26f |
| reverse-migration | reverse-step | merged | 6a50c2d |
| run-vocabulary | caller-neutral-stop | merged | 00365f8 |
| soak | empty-vs-unread | merged | 2219b7e |
| soak | surface-unreachable | merged | 0d7dede |
| soak | vault-write-warning | merged | b78e1d1 |
| soak | verify-fix-licence | merged | 5452e55 |
| standards-bullets | standards-audit | merged | 8bca8cf |
| sync-hazard | sibling-delete | merged | 924eeb0 |
| sync-hazard | sync-doc | merged | 60a3001 |
| sync-hazard | sync-ignores | merged | 92c0b29 |
| third-round | rework-check | merged | 20c098e |
| tracker | slice1 | merged | 6527694 |
| tracker | slice2 | merged | c2bd73b |
| tracker | slice3 | merged | 2df4063 |
| tracker-migration | docs | merged | 379ebce |
| tracker-migration | home-sentinel | merged | a1e1a8c |
| tracker-migration | migrate-run | merged | b5dfcdf |
| tracker-migration | record-count | merged | bb50001 |
| tracker-migration | setup-migrate | merged | edfa586 |
| unchecked-architect | architect-duties | merged | c488159 |
| unchecked-architect | review-boundary | merged | 41169ac |
| agent-model-tuning | override-spawn | planned | |
| agent-model-tuning | agent-models | planned | |
